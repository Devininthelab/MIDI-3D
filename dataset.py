...
class SyntheticDataset(Dataset):
    def __init__(
        ...
        self.dataset_root = os.path.join(root_dir, category)
        # take only the prompts that got availabel non uniform objects
        self.prompts = prompts if prompts is not None else [d for d in os.listdir(self.dataset_root) if os.path.exists(os.path.join(self.dataset_root, d, "available_images.npy"))]
    ...
    def __getitem__(self, index):
        if self.sample_from_all_multiviews:# Boolean flag - if True, samples from all available multi-view images using model_info.npz; if False, uses available_images.npy
            while True: 
                try: 
                    while True: 
                        prompt = random.choice(self.prompts)
                        prompt_root = os.path.join(self.dataset_root, prompt)
                        if not os.path.exists(os.path.join(prompt_root, "model_info.npz")):
                            continue
                        model_info = np.load(os.path.join(prompt_root, "model_info.npz"))
                        ...
                        # should have at least one stable and one unstable model
                    ...
                    # CHECK AGAIN  LOAD CONDITION ALSO INCLUDE SEGMENTATION + IMAGE INFORMATION
                    cond_path = os.path.join(prompt_root, f"{image_id:03d}_cond.pt")
                    ...
                    image_id_lose = image_ids[bad_index]
                    model_win = glb_ids[good_index]
                    model_loss = glb_ids[bad_index]

                    model_win_sparse_x0_path = os.path.join(prompt_root, f"{image_id_win:03d}_sparse_sample_{model_win:03d}.pt")
                    model_loss_sparse_x0_path = os.path.join(prompt_root, f"{image_id_lose:03d}_sparse_sample_{model_loss:03d}.pt")
            ...
            while True:
                prompt = random.choice(self.prompts)
                prompt_root = os.path.join(self.dataset_root, prompt)
                if not os.path.exists(os.path.join(prompt_root, "available_images.npy")):
                    continue
                available_images = np.load(os.path.join(prompt_root, "available_images.npy"))
            ...
            angle_path = os.path.join(prompt_root, f"{image_id:03d}_angles.npy")
            ...
            losing_candidate_ids = good_model_ids[angles[good_model_ids, 0] >= self.stable_threshold]
            assert len(winning_candidate_ids) > 0 and len(losing_candidate_ids) > 0
            model_win = np.random.choice(winning_candidate_ids)
            model_loss = np.random.choice(losing_candidate_ids)

            model_win_sparse_x0_path = os.path.join(prompt_root, f"{image_id:03d}_sparse_sample_{model_win:03d}.pt")
            model_loss_sparse_x0_path = os.path.join(prompt_root, f"{image_id:03d}_sparse_sample_{model_loss:03d}.pt")
